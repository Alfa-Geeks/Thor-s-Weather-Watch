    ...
    <>
        <div className='w-full  bg-gray-900 flex'>
            ...
            <div className='w-[60%]'>
                ...
                {weatherData && (<div className='w-full h-screen'>
                    <div className= 'bg-gray-700 rounded-3xl p-10 mx-10'>
                        <div className='w-full flex gap-16'>
                            <img src={weatherData.current.condition.icon} alt="" />
                            <div>
                                <p className='text-3xl text-white'>{weatherData.location.name}</p>
                                <p className='text-lg text-white'>{weatherData.location.country}</p>
                            </div>
                            <div>
                        ...
                        {hourlyForecast.length > 0 ? (hourlyForecast.map((element) => {
                            return (
                                (
                                    <div className='min-w-16  bg-white rounded-3xl mb-3'>
                                        <p className='text-lg text-black px-3 pt-3'
                                        ...
                                        order:
                                            ...
                                            {new Date(element.time_epoch * 1000).getHours()}:00</p>
                                        <img src={element.condition.icon} alt="" className='w-20'/>
                                        <p className='text-xl text-black px-3 pb-3'>{element.temp_c}
                                            <sup>o</sup>
                                        </p>
                                    </div>
            ...
            <div className=' bg-gray-600 rounded-3xl px-11 pt-10 mx-10 w-[34%] mb-4'>
                <p className='text-3xl text-white font-semibold pb-7'>7 Days Forecast</p>

                {weeklyForecast.length>0 ? (weeklyForecast.map((element)=> {
                    return (
                        <>
                            <div className='mx-2 w-[93%] my-5  h-20 bg-gray-900 rounded-3xl flex justify-between'>
                                <img src={element.day.condition.icon} alt="" className='py-2 px-3'/>
                                <p className='text-white text-3xl my-auto'>
                                    +{Math.round(element.day.maxtemp_c)}
                                    <sup className=''>o</sup>
                                    <span className='text-lg'>/ +{Math.round(element.day.mintemp_c)}</span>
                                </p>
